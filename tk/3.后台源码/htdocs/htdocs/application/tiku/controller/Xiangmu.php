...
//错题学习
   ...
   if(input('post.kk')==1)
	{
		$tid=db('record')->where('mid='.$mid)->limit(1)->order('pid asc')->value('pid');	
	...
	public function zhangjiexuexi()
		{
		 ...
		 if($kk==1)
		 {
			$subject_id=input('post.wid')?input('post.wid'):'';
			if($subject_id!='')
			{
			...
			$info=db('problem')->alias('p')->where('p.id='.$tid)->order('p.id asc')->find();
			$info['answer']=db('answer')->where('pid='.$info['id'])->column('answer');
			$info['intro']=db('answer')->where('pid='.$info['id'])->value('intro');
			$info['options']=db('options')->field('linecode as value,content as label')->where('pid='.$info['id'])->select();			
...
//成绩
		public function getstore(){
			$mid=input('post.userid'); 
			$sjtkid=input('post.sjtkid');		
			$arrid=db('suiji')->where('Id='.$sjtkid)->find();
			$timuid=$arrid['tids'];
			$wid=$arrid['subject_id'];
			$arr=explode(',',$timuid);
			$tishu=count($arr);
			$shitiinfo=[];
			$zhengque=0;
...
//成绩查询
	...
	{
		$mid=input('userid');
		$info=db('score')->join('subject','score.sub_id=subject.id')->field('score.*,subject.subname')->where('mid='.$mid)->select();
		return json($info);
	}
